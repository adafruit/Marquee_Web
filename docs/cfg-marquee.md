...
> **Status: no producer in this repo.** `buildMarqueeCfg()` lived in
> `public/js/cfg.js` and was assembled into a downloadable ZIP by
> `public/js/bundle.js`, reachable from screen A6. A6 was parked before the first
> commit — it was never initialised by `main.js` and imported router exports that
> no longer existed — so both modules were unreachable dead code and were removed.
> This document is retained as the **format spec**: it is what a bundle builder
> must emit and what `code.py` must read. The paths below refer to that removed
> code and are historical.

- **Producer:** *(removed)* `buildMarqueeCfg()` in the former `public/js/cfg.js`.
  Sourced from Act I of the wizard (screens A3–A5).
...
`canvas.json` (`public/js/core/doc.js`), which carries no pins or identity.
...
response (`public/js/device/device.js`); on the CircuitPython path it is published as
...
| `panel` | string | `{size}-{color_mode}-{rev}` (e.g. `213-quad-AJHE5`) for a bare panel, or `{vendor}-{product}` (e.g. `adafruit-magtag`, `xteink-x4-pro`) for one that ships inside a product. |
...
`ifaceTypeFor()` in `public/js/device/presets.js`; a panel id the editor does not know is
`spi_epd`.
...
| `UC8279` | — | — | | no `adafruit_epd.uc8279` at `main` — WipperSnapper path only |
...
The table lives in `EPD_DRIVERS` in `public/js/device/presets.js`; `driverFor(driver,
mode)` does the mode resolution and returns the constructor kwargs alongside.

## What the catalog emits

All nine presets, as produced by `buildMarqueeCfg()`:
...
| `x4pro` | 800×400 | 0 | 800×400 | mono | — | `spi_epd` | none — `UC8279` has no `adafruit_epd` class |

`x4pro` is the only entry whose `driver` the table cannot resolve, so it is the only
one whose bundle carries no usable `code.py` import — the panel is reachable over the
WipperSnapper path, where `driver` is a free-form string the firmware matches, and a
CircuitPython consumer should refuse it rather than guess a class. It is also the only
entry not wired to a Feather: its pins are the X4 Pro's own ESP32-S3 GPIOs.
...
Not described by this file, but a consumer needs it, so: `server/index.js` publishes a
...
The hexes are `PALETTES` in `public/js/canvas/palette.js`, extracted from the committed
remap PNGs in `server/palettes/`.
...
`buildDisplayBody()` in `public/js/core/config.js` serialises the same facts for
