...
Three contracts between the editor, Adafruit IO and the board. Each is
...
All three are plain Adafruit IO feeds under one group key: the editor publishes
the scene and the sleep window, the board publishes its status, and nothing in
between holds state. For how the pieces fit together, see the
