...
- **Producer:** `pushToDisplayCircuitPython()` in `public/js/device/device.js`, via
...
WipperSnapper path, where `server/index.js` implements `TimerConfig` and defers
...
`sleepModeFor()` in `public/js/core/config.js`, mirrored in `server/index.js` for the
...
- **`code.py` does not read this feed yet.** The generated bundle still sleeps on
  ...
  entirely, so the publisher side is complete and the round trip is not. The
  `TODO` in `codePy()` that carried this contract inline went with
  `public/js/bundle.js` when the parked bundle builder was removed — see the
  status note in [`cfg-marquee.md`](cfg-marquee.md). This spec is now the only
  copy of the contract for whoever writes the consumer.
